...
function Spells() {
	...
	async function loadMore() {
		...
		try {
			for (
				var i = numberOfSpells;
				i < numberOfSpells + requestSpells;
				i++
			) {
				let aux = {};
				aux.name = spells[i].name;
				aux.description = (
					await api.get(spells[i].url.substring(5))
				).data.desc;
				console.log(aux);
				cSpells[i] = aux;
			}
			...
			return (
				setCurrentSpells(cSpells),
				setNumberOfSpells(numberOfSpells + requestSpells)
			);
		} catch (err) {
		...
		async function fetchData() {
			try {
				...
				for (var i = numberOfSpells; i < numberOfSpells + 25; i++) {
					...
					aux.description = (
						await api.get(results[i].url.substring(5))
					).data.desc;
					cSpells[i] = aux;
				}
				//console.log(cSpells);
				...
				return (
					setSpells(results),
					setCurrentSpells(cSpells),
					setNumberOfSpells(numberOfSpells + 25),
					setTotalSpells(response.data.count)
				);
			} catch (err) {
				console.error("Requisição falhou! " + err);
			}
